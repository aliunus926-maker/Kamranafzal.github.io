...
$(function () {
  ...
  $.getJSON("portfolio.json")
    .done(function (folders) {
      ...
      folders.forEach(function (folder) {
        const $card = $(`
          <a class="folder-card reveal" href="${escapeHtml(folder.url)}"
             target="_blank" rel="noopener"
             aria-label="Open ${escapeHtml(folder.name)} folder in a new tab">
            <div class="folder-thumb">
              <img src="${escapeHtml(folder.image)}" alt="" loading="lazy" />
              <div class="folder-thumb-overlay" aria-hidden="true"></div>
              <div class="folder-icon" aria-hidden="true">${folder.icon}</div>
            </div>
            <div class="folder-body">
              <h3>${escapeHtml(folder.name)}</h3>
              <span class="btn btn-card" aria-hidden="true">
  ...
  function renderResearch(items, category, targetSelector) {
    ...
    matches.forEach(function (item) {
      ...
      const cardClass = isFeatured ? "research-card research-card-featured reveal" : "research-card reveal";
      const thumb = item.image
        ? `<div class="research-thumb">
             <img src="${escapeHtml(item.image)}" alt="" loading="lazy" />
             <div class="research-thumb-overlay" aria-hidden="true"></div>
             ${isFeatured ? '<span class="featured-badge"><i class="fa-solid fa-star" aria-hidden="true"></i> Featured Documentary</span>' : ""}
           </div>`
        : "";
